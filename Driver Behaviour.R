...
# Data<-stream_in(file("C:/CACoE/CoE Projects/Driving_Pattern/Driving_Pattern.json"))
# write.csv(Data,"D:/Driving_Pattern/Driving_Pattern_Data_Raw.csv")


#Read Excel File & Prepare Data file

Data_Full<-loadWorkbook("C:/CACoE/CoE Projects/Driving_Pattern/Data_Final1.xlsx")
Data_Full<-readWorksheet(Data_Full,sheet = "Final")
colnames(Data_Full)
...
##Selecting K through Hierarchical cluster

d <- dist(as.matrix(Data))
HClst<-hclust(d, method = "complete", members = NULL)
summary(HClst)
plot(HClst)
